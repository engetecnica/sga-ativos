...
class Ferramental_estoque  extends MY_Controller {
    ...
    // public function renovar_items_retirada($id_retirada=null, $id_retirada_item=null, $data=null)
    // {


        

    //         $item_renovar = $this->ferramental_estoque_model->get_ativos_item($id_retirada_item); // Item que será renovado


    //         $item_renovado['id_retirada'] = $item_renovar->id_retirada;
    //         $item_renovado['id_ativo_externo_grupo'] = $item_renovar->id_ativo_externo;
    //         $item_renovado['data_retirada'] = date("Y-m-d H:i:s");
    //         $item_renovado['data_devolucao_prevista'] = date("Y-m-d H:i", strtotime($data));
    //         $item_renovado['quantidade'] = 1;
    //         $item_renovado['status'] = 4; // Recebido

    //         // if (strtotime($item_renovado['devolucao_prevista']) < strtotime('now')) {
    //         //     $this->session->set_flashdata('msg_erro', "Data de devolução prevista deve ser maior que atual!");
    //         //     echo redirect(base_url("ferramental_estoque/detalhes/{$id_retirada}"));
    //         //     return;
    //         // }     

    //         $id_retirada_item_renovado = $this->ferramental_estoque_model->salvar_retirada_renovacao_item($item_renovado);



    //         $ativos['id_retirada'] = $id_retirada;
    //         $ativos['id_retirada_item'] = $id_retirada_item_renovado;
    //         $ativos['id_ativo_externo'] = $item_renovar->id_ativo_externo;
    //         $ativos['status'] = 4;
    //         $ativos['data_retirada'] = date("Y-m-d H:i:s");


    //         $this->db->insert("ativo_externo_retirada_ativo", $ativos);



    //     $this->session->set_flashdata('msg_success', "Renovação feita com sucesso!");
    //     echo redirect(base_url("ferramental_estoque/detalhes/{$id_retirada}"));
    ...
    // public function devolver_items_renovacao($id_retirada, $id_retirada_item, $ativo_id_retirada_item)
    // {

    //     // esse item já foi devolvido 

    //     if(isset($id_retirada) && isset($id_retirada_item)){
    //         $this->ferramental_estoque_model->devolver_items_renovacao($id_retirada_item);
    //         $this->session->set_flashdata('msg_success', "Item devolvido com sucesso!");
    //         echo redirect(base_url("ferramental_estoque/detalhes_item/{$id_retirada}/{$ativo_id_retirada_item}"));             
    //     } else {
    //         $this->session->set_flashdata('msg_erro', "Item não encontrado!");
    //         echo redirect(base_url("ferramental_estoque"));            
    //     }
    // }
