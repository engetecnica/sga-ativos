            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form id="form-retirada" action="<?php echo base_url('ferramental_estoque/salvar'); ?>"
                                method="post" enctype="multipart/form-data">

                                <h3 class="title-2 m-b-20 m-t-25">Funcionário Solicitante</h3>
                                <!-- Detalhes da Retirada-->
                                <table
                                    class="table dataTable table-responsive-md table--no-card table-borderles table-striped table-earning  m-b-40">
                                    <thead>
                                        <tr class="active">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>
                                                ...
                                                <v-select class="select form-control col-12"
                                                    :value="funcionario_seleted_value" :options="funcionarios_filted"
                                                    ...
                                                    <template v-slot:no-options="{ search, searching }">
                                                        ...
                                                        <em v-else style="opacity: 0.5">Buscar Funcionário via Nome,
                                                            Matrícula, RG ou CPF</em>
                                                    </template>
                                                </v-select>
                                            </td>
                                ...
                                <table
                                    ...
                                    <thead>
                                        <tr class="active">
                                            <th scope="col" width="100%">Buscar Item</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>
                                                <select id="sl2multi" class="select select form-control " multiple  style="width:300px!important">
                                ...
                                <input v-if="retirada" type="hidden" name="id_retirada" id="id_retirada"
                                ...
                                <input type="hidden" id="solicitar_autorizacao" name="solicitar_autorizacao"
                                    :value="solicitar_autorizacao">
                                ...
                                <div class="row form-group m-t-40">
                                    ...
                                    <div class="col-12 col-md-3">
                                        <input require="required" type="datetime-local" class="form-control"
                                            name="devolucao_prevista" id="devolucao_prevista"
                                            v-model="devolucao_prevista">
                                    </div>
                                    ...
                                    <div class="col-12 col-md-4">
                                        <textarea v-model="observacoes" class="form-control" name="observacoes"
                                            id="observacoes" placeholder="Alguma observação?">
                                        </textarea>
                                    </div>
                                </div>
                                ...
                                <hr>
                                <div class="pull-left">
                                    <button type="submit" form="form-retirada" class="btn btn-primary"
                                        :disabled="permite_form">
...
<script>
    $(document).ready(function(){
        $("#sl2multi").select2({closeOnSelect:false});
    })
...
var estoque = new Vue({
    ...
    computed: {
        ...
        grupos_filted() {
            return this.grupos.filter((gp) => {
                return !this.inGruposSelecionados(gp.id_ativo_externo_grupo)
            });
        },
        permite_form() {
            return ![
    ...
    methods: {
        inGruposSelecionados(id_grupo) {
            return this.grupos_selecionados.find((gp) => {
                return gp.id_ativo_externo_grupo == id_grupo
            });
        },
        filterGrupos(grupos) {
            // return grupos
        ...
        addItem(item) {
            ...
            if (array_patrimonio.length > 0) {
                ...
                const beforeIndex = this.grupos_selecionados.findIndex(i => {
                    return i.id_ativo_externo_grupo == item?.id_ativo_externo_grupo || i
                        .id_ativo_externo_grupo == item
                });

                if (item && beforeIndex < 0) this.grupos_selecionados.push({
                    ...item,
                    quantidade: 1,
                    array_patrimonio
                });
        ...
        removeItem(item, codpat) {
            ...
            if (typeof item !== 'object') item = this.grupos_selecionados.find(i => i.id_ativo_externo_grupo ==
                item)
                ...
                return window.$.ajax({
                    ...
                    .done(function(lista) {
                        const retiradas = Object.values(JSON.parse(lista) || [])
                            .filter(r => (new Date(r.devolucao_prevista)).getTime() < (new Date())
                            .getTime())

                        if (estoque.funcionarios.length) {
                            const index_funcionario = estoque?.funcionarios.findIndex(f => f
                        ...
                        const msg = estoque.user.nivel == 1 ? "Deseja continuar e autorizar retirada?" :
                            "Deseja continuar aguardando confirmação de um Administrador?"
