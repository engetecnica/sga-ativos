    ...
    <div class="btn-group">
        ...
        <div class="dropdown-menu">
            <?php if ($this->permitido($permissoes, 10, 'editar')) { ?>
                ...
                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="<?php echo base_url("ativo_interno/qrcode/{$row->id_ativo_interno}"); ?>">
                    <i class="fa fa-qrcode"></i>&nbsp; Gerar Etiqueta
                </a>

                <?php if ((int) $row->situacao < 2) { ?>
                    <div class="dropdown-divider"></div>
                    <a class="dropdown-item  confirmar_registro" href="javascript:void(0)" class="confirmar_registro" data-registro="<?php echo $row->id_ativo_interno; ?>" data-href="<?php echo base_url("ativo_interno/descartar/{$row->id_ativo_interno}"); ?>" data-tabela="<?php echo base_url("ativo_interno"); ?>" data-icon="info" data-message="false" data-acao="Descartar" data-title="Confirmar descarte do ativo" data-redirect="true" data-text="Clique 'Sim, Confirmar!' para confirmar o descarte do ativo.">
            ...
            <?php if ($this->permitido($permissoes, 10, 'excluir')) { ?>
                <?php if ((int) $row->situacao == 2 && !isset($row->data_descarte)) { ?>
                    <div class="dropdown-divider"></div>
                    <a class="dropdown-item  deletar_registro" href="javascript:void(0)" data-href="<?php echo base_url('ativo_interno'); ?>/deletar/<?php echo $row->id_ativo_interno; ?>" data-registro="<?php echo $row->id_ativo_interno; ?>" data-tabela="ativo_interno">
