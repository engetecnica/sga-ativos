...
$sheet->setCellValue('E2', 'Horimetro Inicial');
$sheet->setCellValue('F2', 'Horimetro Atual');
$sheet->setCellValue('G2', 'Horimetro Última Revisão');
$sheet->setCellValue('H2', 'Horimetro Próxima Revisão');
$sheet->setCellValue('I2', 'Horimetro Rodados');
...
if($data){
    foreach($data as $i=>$row){
        ...
        $sheet->setCellValue("E{$sheet->rows}", $row->horimetro_inicial ?: 0);
        $sheet->setCellValue("G{$sheet->rows}", $row->horimetro_atual ?: $row->horimetro_atual);
        $sheet->setCellValue("H{$sheet->rows}", $row->horimetro_ultima_revisao ?: 0);
        $sheet->setCellValue("I{$sheet->rows}", $row->horimetro_proxima_revisao ?: 0);
        $sheet->setCellValue("I{$sheet->rows}", $row->horimetro_rodado ?: 0);
