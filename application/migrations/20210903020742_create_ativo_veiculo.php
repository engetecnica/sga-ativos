...
class Migration_Create_Ativo_Veiculo extends CI_Migration {
	...
	public function up(){
		if (!$this->db->table_exists($this->table)) {
			...
			->add_field("tipo_veiculo enum('carro','moto','caminhao') NOT NULL DEFAULT 'carro'")
			->add_field('id_marca int(10) NOT NULL DEFAULT 0')
			->add_field("id_modelo varchar(10) NOT NULL DEFAULT '0'")
			->add_field('ano varchar(10) NOT NULL')
			->add_field('veiculo varchar(255) NOT NULL')
			->add_field('valor_fipe float NOT NULL')
			->add_field('codigo_fipe varchar(50) NOT NULL')
			->add_field('fipe_mes_referencia varchar(100) NOT NULL')
			->add_field('veiculo_placa varchar(20) NOT NULL')
			->add_field('veiculo_renavam varchar(255) NOT NULL')
			->add_field('veiculo_km varchar(50) NOT NULL')
			->add_field('veiculo_km_data date NOT NULL')
			->add_field('valor_funcionario float NOT NULL')
			->add_field('valor_adicional float NOT NULL')
